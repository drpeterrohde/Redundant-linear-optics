...
First let us consider a general linear transformation described by a unitary network matrix $\hat{U}$. Theorem~\ref{Theorem 1} given access to $N$ linear networks which are randomly distributed but close to $\hat{U}$ that the standard estimator for the mean values of these unitaries approaches $\hat{U}$ in the same sense as the central limit theorem.
