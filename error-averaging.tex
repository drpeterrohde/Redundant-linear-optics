...
\end{equation}
Consider a target matrix $U$ which commutes with the single error such that $M=Ue^{i\theta T}$, and assume a Gaussian error distribution with variance $\sigma^2$. Then provided $T$ is such that $\theta=0$ %I dont think this is necessary, i suspect the commuting error requirement forces this.% 
