...
\begin{proof}
	...
	\begin{equation}
		M = \int^\pi_{-\pi} e^{i\theta} p(\theta) \mathrm{d}\theta.
	\end{equation}
...
\end{corollary}
\begin{proof}
	We will extend the proof of Corollary~\ref{Corollary 1} to the $n$-dimensional case.  Write $U_k = U exp\{i t \sum_l \alpha_{kl} T_l\}$ where $T_l$ are the $n^2-1$ generators of the $u(n)$ Lie-algebra and $U$ is chosen so that the distribution is defined by probability density function $p(\alpha_1,\ldots,\alpha_{n^{2}-1})$ is peaked around the origin. $T_l$ are chosen so that and $-\pi < \alpha_l < \pi$.
\begin{widetext}
	\begin{equation}
		M = U
		\int^\pi_{-\pi} \ldots \int^\pi_{-\pi} \int^\pi_{-\pi}
		exp\{i t\sum_l \alpha_{l} T_l\} p(\alpha_1,\ldots,\alpha_{n^{2}-1})
		\mathrm{d}\alpha_1
		\mathrm{d}\alpha_2
		\ldots
		\mathrm{d}\alpha_{n^{2}-1} \label{eq:general integral form}
	\end{equation}
	\begin{equation}
		M = U
		\int^\pi_{-\pi} \ldots \int^\pi_{-\pi} \int^\pi_{-\pi}
		(1 + i t\sum_l \alpha_{l} T_l + O(t^2)) p(\alpha_1,\ldots,\alpha_{n^{2}-1})
		\mathrm{d}\alpha_1
		\mathrm{d}\alpha_2
		\ldots
		\mathrm{d}\alpha_{n^{2}-1}
	\end{equation}
\end{widetext}

	Consider the case where $p(\alpha_1,\ldots,\alpha_{n^{2}-1}) = p(\alpha_1)\times\ldots\times p(\alpha_{n^{2}-1})$ is a sufficiently narrow distribution around $\alpha_{l}=0$ such that $[a_{kl}T_{l},a_{km}T_{m}]\approx0 \forall l,m$. Equation \ref{eq:general integral form} simplifies by the Baker-Campbell-Hausdorff formula to
...
\begin{widetext}
	...
	\begin{equation}
		...
		exp\{i t\alpha_{n^{2}-1} T_n^{2}-1\}p(\alpha_n^{2}-1)
	\end{equation}
\end{widetext}
