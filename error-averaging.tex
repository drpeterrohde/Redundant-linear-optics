...
\end{corollary}
\begin{proof}
	We will extend the proof of Corollary~\cite{Corollary 1} to the $n$-dimensional case.  Write $U_k = U exp\{i t \sum_l \alpha_{kl} T_l\}$ where $T_l$ are the $n^2-1$ generators of the $u(n)$ Lie-algebra and $U$ is chosen so that the distribution is defined by probability density function $p(\alpha_1,\ldots,\alpha_l)$ is peaked around the origin. $T_l$ are chosen so that and $-\pi < \alpha_l < \pi$.
\begin{widetext}
	\begin{equation}
		M = U
		\int^\pi_{-\pi} \ldots \int^\pi_{-\pi} \int^\pi_{-\pi}
		exp\{i t\sum_l \alpha_{l} T_l\} p(\alpha_1,\ldots,\alpha_l)
		\mathrm{d}\alpha_1
		\mathrm{d}\alpha_2
		\ldots
		\mathrm{d}\alpha_l
	...
	\begin{equation}
		M = U
		\int^\pi_{-\pi} \ldots \int^\pi_{-\pi} \int^\pi_{-\pi}
		(1 + i t\sum_l \alpha_{l} T_l + O(t^2)) p(\alpha_1,\ldots,\alpha_l)
		\mathrm{d}\alpha_1
		\mathrm{d}\alpha_2
		\ldots
		\mathrm{d}\alpha_l
...
To implement the averaging of the random unitary network matrices from Theorem~\cite{Theorem 1} we utilise a linear network whose elements are those of a Discrete Fourier Transform (DFT).  That is, we have an input-output relationship between mode annihilation operators of the form
\begin{equation}
