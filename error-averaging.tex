...
\end{align}
where $\bar{p}(\theta) = p(\mu + \theta)$ so that it has mean value zero.  We now only consider the case of distributions that have mean zero.  By expanding the matrix exponential this expression can be written as
\begin{equation}
	M=\sum_n \frac{(iT)^n}{n!} \int \theta^n p(\theta) d\theta,
\end{equation}
which now relates to the moments of the underlying distribution in $\theta$.  If $p(\theta)$ were a Gaussian distribution with variance $\sigma^2$ then we can write
\begin{equation}
...
\begin{corollary}
\label{Corollary 2}
	If $\{U_1,\ldots,U_N\}$ are $n$-dimensional but with a narrow distribution so that they all approximately commute with each other, then $M = c U$ where $0 < c < 1$ and $U$ is a unitary matrix.
\end{corollary}
\begin{proof}
