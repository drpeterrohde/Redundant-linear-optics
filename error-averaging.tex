...
%\thanks{}
%\altaffiliation{}
\email{ryan.marshman@uqconnect.edu.au}
\affiliation{Centre for Quantum Computation and Communication Technology (CQC2T), The University of Queensland, Australia.}

\author{Austin Lund}
\affiliation{Centre for Quantum Computation and Communication Technology (CQC2T), The University of Queensland, Australia.}

\author{Peter Rohde}
\affiliation{Centre for Quantum Computing \& Intelligent Systems (QCIS), University of Technology Sydney, Australia.}
...
\section{General Results \label{gen case}}
	First let us consider a very general system. This will serve as a basis to explore the properties and effect of error averaging.
	...
	\begin{equation}
		\hat{U}_{effective}=\frac{1}{N}\sum_{i=1}^{N}\hat{U}_{i}
	\end{equation}
	...
	\subsection{A General System \label{gen system}} 
		...
		To implement error averaging on this system $N$ copies of $\hat{U}$ are made, denoted here as $\hat{U}_1, \hat{U}_2, ..., \hat{U}_N$. The input state is then placed in an even superposition of $N$ spacial modes, each of which implement one of the copies of $\hat{U}$. This forms the second formal requirement on the implementation, specifically the input particles must be able to be placed in a spacial superposition. These spacial modes are then recombined by applying the splitting mechanism used to encode the superposition in reverse. Errors can then be filtered out by post selecting on all extra modes left over from the splitting process being in the vacuum state. Figure \ref{fig:gen system} shows how this can be implemented using linear optics. Figure \ref{fig:MZ_setup} highlights the location of the error detection modes and photo-detectors.
		
		Figure \ref{fig:gen system} also demonstrates that the encoding used to implement error averaging is naturally self correcting until the outermost layer, with the inner layers being error averaged by the outer layers.
		...
		Considering a system with $N=2$, that is, an averaged system with only two copies of the intended transformation. This system's transformation can be defined by taking $u_{i}$ as the $n\times n$ matrix representation of $\hat{U}_{i}$ and setting 
		\begin{equation}
			U_{1} = \begin{bmatrix} u_{1} & 0         \\
									   0 & \mathbb{I}
				   	\end{bmatrix}
		\end{equation}
		...
		\begin{equation}
			U_{2}=\begin{bmatrix}\mbox{\ensuremath{\mathbb{I}}} & 0\\
				0 & u_{2}
			\end{bmatrix}
		\end{equation}
		...
		\begin{equation}
			S=\frac{1}{\sqrt{2}}\begin{bmatrix}\mbox{\ensuremath{\mathbb{I}}} & \ensuremath{-\mathbb{I}}\\
		...
		\begin{equation}
			\left|\phi\right\rangle =\left|\phi_{1},...,\phi_{n}\right\rangle \equiv\begin{bmatrix}\phi_{1}\\
			\phi_{2}\\
			\vdots\\
			\phi_{n}
			\end{bmatrix}=\vec{\phi}
...
\section{Implementation \label{implementation}}
	...
	\subsection{1 photon N arbitrary \label{1 photon N arbitrary}}
		...
		It is also important to note the probability of success, in this case it is given by
		...
		\begin{figure}
			...
			\caption{\label{fig:post vs no post}Probability of observing the photon number at the $a$	output port as shown in Figure \ref{fig:MZ_setup}. Or equivalently expected value of the photon number at the $a$ output port. Note that this is equivalent to the probability of the photon coming out in the correct mode with and without post selection. Here $v=0.1\ \textrm{rad}^{2}$, the orange values correspond to with post selection and blue corresponds to without post selection.}
		\end{figure}
	\subsection{2 photons N arbitrary \label{2 photons N arbitrary}}
	
		Given the highly quantum nature of two photon inference it was also be discussed. This also serves to demonstrate that error averaging is applicable to a wide variety of systems. 
		
		For two photons we have a slightly more complicated but similar general form for the output state as shown in Eq.\ref{eq:2pNarbitrary S}, again only considering the $a$ and $b$ output modes, but not considering the re-normalisation which occurs with post selection.
	...
	\subsection{Comparison between averaging at end and averaging each step\label{averaging at end vs step}}
		...
		To compare the effect of averaging across the system and averaging each step a single Mach-Zehnder interferometer as a Hong-Ou-
		Mandel (HOM) like system was considered. The HOM system demonstrates two photon interference whereby the two photon state $\left|1,1\right>$ is fed into a system and never observed as an output due to quantum interference \cite{hom}. This was chosen as it generally serves as a standard test bench for quantum optics because it depending on the quality of optical components and how well the two	individual photons can be distinguished the probability of observing the coincidence state, $\left|1,1\right>$, varies. The standard coincidence input
		
		\begin{equation}
		\left|\psi\right> = \left|1,1\right>
		\end{equation}
		
		was considered and the probability of observing this state in the output as a function of the
		applied angle $\theta$ determined. By considering three different systems, a system with no error averaging (included for comparison), the standard beam splitter with minimal error averaging and our new system of averaging across the system. Both averaged circuits have two phase shifters and so give a valid way of testing the new system. Figure 3.6 shows the diagrammatic set up of all three systems.
		Table \ref{tab:Hong-Ou-Mandel-systems-output} shows the results for the three HOM set-ups. What can be seen is that we again get a benefit from error averaging with post selection. Interestingly this also suggest averaging across the system has the same result as averaging the beam splitter. The results show the same pattern as the one and two photon systems above whereby averaging simply halves the error term. This is promising for the averaging across the system method of correction. This method is of interest as it could potentially protect against errors in the fixed beam splitters as well as phase shifters as it is averaging across all but the outer most beam splitters. This is significant as the main benefit of this method is seen for large N in which case there are a significant number of fixed beam splitters which have so far been considered perfect.
		It is hoped that the requirement of these to all be perfect can then be relaxed allowing these results to hold more generally and be more applicable to currently manufacturable	systems. Note that by allowing an arbitrary phase shift this result again serves to verify that simply considering identity transformations is suitable to characterise the error when averaging across the beam splitter or across the system.
		
		\begin{table*}
			\caption{Hong-Ou-Mandel systems output results. The top line shows the probability of the coincidence result while the bottom line shows that same probability	once all error ports have been post selected on. Once again $v$ is the variance of the beam splitter error and $\theta$ is the intended	applied phase shift. \label{tab:Hong-Ou-Mandel-systems-output}}
			\resizebox{\textwidth}{!}{%
				\begin{tabular}{|c|c|c|c|}
				\hline 
					& No Averaging & Averaging in the beam splitter & Averaging across the system\tabularnewline
					\hline 
					\hline 
					$P\left(\textrm{DesiredOutput}\right)$ & $\frac{1}{2}\left(1+\cos\left(2\theta\right)\right)-v\cos\left(2\theta\right)$ & $\frac{1}{2}\left(1+\cos\left(2\theta\right)\right)-\frac{v}{4}\left(1+3\cos\left(2\theta\right)\right)$ & $\frac{1}{2}\left(1+\cos\left(2\theta\right)\right)-\frac{v}{4}\left(1+3\cos\left(2\theta\right)\right)$\tabularnewline
					\hline 
					$P\left(\textrm{DesiredOutput}\right)$with post selection & $\frac{1}{2}\left(1+\cos\left(2\theta\right)\right)-v\cos\left(2\theta\right)$ & $\frac{1}{2}\left(1+\cos\left(2\theta\right)\right)-\frac{v}{2}\cos\left(2\theta\right)$ & $\frac{1}{2}\left(1+\cos\left(2\theta\right)\right)-\frac{v}{2}\cos\left(2\theta\right)$\tabularnewline
					\hline 
				\end{tabular}			
				}
		\end{table*}

		For error averaging to be more useful, it will need to protect against more than simply phase errors. As previously discussed, it is possible to turn beam splitter transformation errors into phase errors by simply using MZ interferometers as the beam splitters and so we can expect		error averaging to reduce errors is these beam splitters too. As it	has been shown that any linear unitary transformation can be implemented using beam splitters and phase shifters in some large semi-complicated arrangement \cite{reck}, error averaging might be suitable for any linear optical circuit.
		...
		The set up being considered is a four mode system consisting of four beam splitters, each implemented as above, that is each being its own MZ interferometer as shown in Figure \ref{fig:4 mode basis diagram}. Three different input states were chosen: a single photon input in	the top mode and the vacuum state at all other modes$\left(\left|1,0,0,0\right\rangle \right)$,	two photons, both in the top mode $\left(\left|2,0,0,0\right\rangle \right)$ and two photons spread across the top two modes $\left(\left|1,1,0,0\right\rangle \right)$. For simplicity the system was chosen to again implement the identity and error reduction was then applied as in the Hong-Ou-Mandel systems discussed above. All results were found using \textit{Mathemtatica} to sample from the appropriate transformation matrix representing the system and then compute the photon number expectation values and probabilities. Figure \ref{fig: averaging 4 mode diagram} is a diagram of the 4 mode system averaged across the system once.
	...
	\subsection{Generalising Example Results}
		...
		This shows that these correction methods appear to have a very simple effect and may be equivalent. The asymptotic limit in the probability	of success shown in Eq.\ref{eq:PsuccessGeneral} is particularly promising as it again demonstrates that there is some limited price for correcting arbitrarily well as shown by Eq.\ref{eq:PcorrectGeneral}. This general pattern matches the result shown in Eq.\ref{eq:1pWithPost} with $N=2^{n-1}$ which is odd given it does not quite match Eq.\ref{eq:2pNarb PS}. More explicitly, the results from Table \ref{tab:Hong-Ou-Mandel-systems-output} suggest the error term in Eq.\ref{eq:2pNarb PS} should be $\frac{v}{N}$ which is twice what was found. This is despite being suitable in both the one and two photon cases for the four mode set-up. However as noted, Eq.\ref{eq:2pNarb PS} holds for $N\ge4,$ so if we take $N=2$ as our initial system, the `initial' error will taken to be half what it was and these results match exactly. Another potential explanation is that we are intending to implement the identity transformation results in the two photons effectively not interfering at all. It is therefore not all that surprising then that Eq.\ref{eq:PcorrectGeneral} matches the previous single photon results, that is Eq.\ref{eq:1pWithPost}, better than the two photon results, that is Eq.\ref{eq:2pNarb PS}.
			
		This result may also allow the correction of errors in the inner most fixed beam splitters. If the once corrected system with error laden beam splitters is taken as the initial step in the sequence then each further step will be averaging across both the fixed beam splitters and the original error laden system. This could allow some of the beam splitters to be corrected  making the base assumptions on the quality of the fixed beam splitters less restrictive. This is highlighted in Figure \ref{fig:gen system} where, depending on which components are considered to be the system, it is averaged either $8$, $4$ or $2$ times. As previously stated, averaging across the system also has a lower resource cost suggesting it is a far more preferable method of error correcting.
