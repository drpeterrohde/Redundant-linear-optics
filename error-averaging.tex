...
Linear bosonic networks, particularly in the optics context, form the most fundamental interactions between particles.  Though linear networks cannot form universal quantum processing devices on their own, they can using post-selection methods. More recently have been shown to generate quantum statistics that cannot be computed using classical computing resources alone.

Linear networks for quantum optics experiements have traditionally been
...
In theoretical proposals for optical quantum information tasks using linear networks, it is often assumed that it is possible to configure a network with high speed and with extremely high precision.  This paper considers the second of these requirements by studing the effects of imprecision in configuring linear networks.

The model we will consider assumes that large linear networks can be configured arbitrarily but with some additional noise applied to that which is desired in such a was as to maintain linearly of the overal operation.  We also assume the ability to generate Fock basis states and make Fock basis detections.  The networks have no loss and the injection of states and detection are ideal.  In this way we focus on the errors introduced due to the imprecision of configuring the network.
...
\end{figure}
The central limit theorem applies to the individal matrix elements of the averaged transformation and hence its variation decreases as $\frac{1}{N}$.  The form of the average matrix and the distribution of the transformations on a finite number of averages depends on the details of the noise applied to the network encoding. 

The next section introduces the scheme and some mathematical details for the most general case. Section~\ref{implementation} includes numerous proof of principle examples which serve to highlight the effects of Error Averaging with a focus on the behaviour of the probability of success. Section~\ref{averaging at end vs step} will study two different ways of redunantly encoding a single mode phase shift and the effects of the different encodings on the resultant error and probability of success.  Numerically we will study the averaging method for a four-mode operation in section~\ref{Four Mode Implementation Comparison}.  We will discuss some of the conseqneces of these results as well as future directions in Section~\ref{Discussion} before making some concluding remarks. 
...
%
% What this _really_ should be about is the generalisation to $N$ modes.  This wan't really numerically simulated as it is a bit hard to do that.
% There are some critial problems here that I'm not sure if they are able to be recovered from:
% a) The model of counting for a ``general'' unitary admits that it under counts.
% b) The bit about AA seems _weird_. What does it mean about `n' times!?  AA say nothing about the parameterisation of their problem.  Not sure where this came from.  But an interesting idea is that n-photons into m-modes must have parameters for which there is no sensitivity in the output probability.  
