...
\end{equation}
%A note to Austin: This is the actual form, the more complex equation with the strange behaviour was due to my intergration bounds
...
\begin{corollary}
\label{Corollary 2}
	If $\{U_1,\ldots,U_N\}$ are $n$-dimensional but with a narrow distribution such that all errors approximately commute and are independent, then $M = c U$ where $0 < c < 1$ and $U$ is a unitary matrix.
\end{corollary}
\begin{proof}
	We will extend the proof of Corollary~\ref{Corollary 1} to the $n$-dimensional case.  Write $U_k = U exp\{i t \sum_l \alpha_{kl} T_l\}$ where $T_l$ are the $n^2$ generators of the $u(n)$ Lie-algebra and $U$ is chosen so that the distribution is defined by probability density function $p(\alpha_1,\ldots,\alpha_{n^{2}-1})$ is peaked around the origin. $T_l$ are chosen so that and $-\pi < \alpha_l < \pi$. As all errors are taken to be independent we can also write $p(\alpha_1,\ldots,\alpha_{n^{2}}) = p(\alpha_1)\times\ldots\times p(\alpha_{n^{2}})$. Finally we are requiring $p\left(\alpha_{i}\right)$ to be a sufficiently narrow distribution around $\alpha_{l}=0$ such that $[a_{kl}T_{l},a_{km}T_{m}]\approx0 \forall l,m$. 
\begin{widetext}
	\begin{equation}
		M = U
	...
	\end{equation}
	\begin{equation}
		M = U\int^\pi_{-\pi} exp\{i t\alpha_{1} T_1\}p(\alpha_1) \mathrm{d}\alpha_1 	\int^\pi_{-\pi} exp\{i t\alpha_{2} T_2\}p(\alpha_2) \mathrm{d}\alpha_2 \ldots \int^\pi_{-\pi}
		exp\{i t\alpha_{n^{2}-1} T_n^{2}-1\}p(\alpha_n^{2}).
	\end{equation}
\end{widetext}
	
	Now given the narrow probability distributions are peaked on zero we can conclude, as in Equation \ref{eq:Gaussian Psuccess}, that
...
The non-approximately commuting solution to Equation \ref{eq:general integral form} is left as an open problem. In such a situation, $M$ is not necessarily a normal matrix which makes any conclusions difficult to be drawn.
...
\end{figure}
Section \ref{implementation} demonstrates how Error Averaging can be implemented for various example systems. These examples also serve as a verification that the assumption of approximately commuting errors is not entirely unreasonable. It can also be noted that Equation \ref{sum_unitary} can become the appropriate transformation for duality quantum computing by allowing the $u_{i}$ to be arbitrary \cite{dualityQC}.
...
\section{Implementation \label{implementation}}
	...
	The following sections will discuss a series of example systems implementing error averaging with a focus on phase errors for simplicity. First note that two of the controllable and fundamental components in LOQC are phase shifters and beam splitters. A paper by Jacques Carolan et. al. \cite{ULO} demonstrated how this can be reduced to controllable phase shifters and fixed beam splitters. The basic idea is to replace each beam splitter with a Mach Zehnder (MZ) interferometer consisting of a controllable phase shifter in one arm and two fixed $50:50$ beam splitters. Although this idea is not particularly new, they showed that such a system is able to implement many quantum logic gates and linear optical protocols with a high fidelity. Given the success of this device it was chosen to form the test bench for Error Averaging. In this way we also only need to consider phase errors to correct a general linear optical quantum computing system.
	...
	\subsection{1 photon N arbitrary \label{1 photon N arbitrary}}
		...
		It is worth noting that in the large $N$ limit, this corresponds to the linear approximation of Equation \ref{eq:Gaussian Psuccess} where we can identify that in general $P(\textrm{success})=c$ where is as above. To further illustrate the effect of error averaging when combined with post selection, consider the case when $\theta=0$, the probability of observing the correct result without post selection is
	...
	\subsubsection{Generalising Example Results}
		...
		The equivalence of the two methods of implementation suggest that all results were once again taken from the situation where all errors approximately commute. The result can also be understood to be the first order approximation to Equation \ref{eq:approx commuting, general case}. The $\frac{a_1+1}{b_1}$ coefficient does not quite match what might be expected from Equation \ref{eq:approx commuting, general case} however it can be understood as a result of commuting all errors past the perfect case. It will also be allowing for there not to be any clear isomorphic map between the parameters in the system and the error coefficients of the Lie algebra generators.
