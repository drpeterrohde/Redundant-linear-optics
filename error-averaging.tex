...
\begin{figure}[tbh]
	\begin{subfigure}[b]{\columnwidth}
		\includegraphics[width=1\columnwidth]{beam_splitter_system_b.pdf}
		\caption{}
...
\end{equation}
The coefficient here represents a reduction in the amplitude should this transformation be applied to a state, and hence represents the probability of success. If one desires a $O(1)$ probability of success, then the operator noise must obey $\sigma = O(k^{-1/2})$ and $\sigma = O(n^{-1})$ as $k,n \rightarrow \infty$.   But a probability of $O(1)$ may not be necessary, depending on the application.  A polynomially decreasing probability is acceptable for many computational tasks. For example in the case of Boson Sampling, exponential decays in the probability can be acceptable depending on the exact details of the distributions of outputs on a failure condition, a consideration not made in this paper.
% Make a comparison with the boson sampling error tolerances
