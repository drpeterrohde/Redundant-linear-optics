...
Linear bosonic networks, particularly in the optics context, form the most fundamental interactions between particles.  Though linear networks cannot form universal quantum processing devices on their own, they can using post-selection methods. More recently they have been shown to generate quantum statistics that cannot be computed using classical computing resources alone.
...
The model we will consider assumes that large linear networks can be configured arbitrarily but with some additional noise applied to that which is desired in such a was as to maintain linearly of the overall operation.  We also assume the ability to generate Fock basis states and make Fock basis detections.  The networks have no loss and the injection of states and detection are ideal.  In this way we focus on the errors introduced due to the imprecision of configuring the network.
