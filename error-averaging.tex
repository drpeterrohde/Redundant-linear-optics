...
This model is not a universal description of error processes that could occur.  Particularly, errors like loss, mode-mismatch and non-linearities are not included. It is possible that there is some component of these processes may be reducible under our technique, it is more likely that their effects be exacerbated.  For this paper, we will focus on the errors within the linear processes.
...
Using the DFT matrix for redundantly encoding and decoding has been used here to illustrate in a compact way the generic approach.  But it is not necessary to perform these operations in exactly this way.  The encoding and decoding will give the desired properties provided the transformation of Eq.~\ref{sum_transformation} is generated.  Figure~\ref{fig:gen system} shows how this encoding can be realised using 50:50 beam splitters for an eight mode system. This approach gives a method of encoding which is recursive and would allow the components implementing the encoding to themselves be redundantly encoded. Given this, although the encoding beam splitters are assumed perfect, this requirement can potentially be relaxed. See Section~\ref{Four Mode Impementation Comparison} for more detail.
